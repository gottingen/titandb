...
namespace titandb {
    ...
    rocksdb::Status RedisDB::GetRawMetadata(const Slice &ns_key, std::string *bytes) {
    ...
    rocksdb::Status RedisDB::GetRawMetadataByUserKey(const Slice &user_key, std::string *bytes) {
        std::string ns_key;
        AppendNamespacePrefix(user_key, &ns_key);
        return GetRawMetadata(ns_key, bytes);
    }

    rocksdb::Status RedisDB::Expire(const Slice &user_key, uint64_t timestamp) {
        std::string ns_key;
        AppendNamespacePrefix(user_key, &ns_key);
    ...
    rocksdb::Status RedisDB::Exists(const std::vector<std::string_view> &keys, int *ret) {
        *ret = 0;
        LatestSnapShot ss(storage_);
        rocksdb::ReadOptions read_options;
        read_options.snapshot = ss.GetSnapShot();
    ...
    rocksdb::Status RedisDB::TTL(const Slice &user_key, int64_t *ttl) {
        std::string ns_key;
        AppendNamespacePrefix(user_key, &ns_key);
    ...
    void RedisDB::GetKeyNumStats(const std::string &prefix, KeyNumStats *stats) { Keys(prefix, nullptr, stats); }

    void RedisDB::Keys(const std::string &prefix, std::vector<std::string> *keys, KeyNumStats *stats) {
        uint16_t slot_id = 0;
    ...
    rocksdb::Status RedisDB::Scan(const Slice &cursor, uint64_t limit, const Slice &prefix,
    ...
    rocksdb::Status RedisDB::RandomKey(const std::string &cursor, std::string *key) {
        ...
        if (!keys.empty()) {
            unsigned int seed = turbo::ToTimeT(turbo::Now());
    ...
    rocksdb::Status RedisDB::Dump(const Slice &user_key, std::vector<std::string> *infos) {
        infos->clear();
    ...
    rocksdb::Status RedisDB::FindKeyRangeWithPrefix(const std::string &prefix, const std::string &prefix_end,
        ...
        read_options.snapshot = ss.GetSnapShot();
        storage_->SetReadOptions(read_options);
        auto iter = UniqueIterator(storage_, read_options, cf_handle);
        iter->Seek(prefix);
    ...
    turbo::Status WriteBatchLogData::Decode(const rocksdb::Slice &blob) {
        const std::string &log_data = blob.ToString();
        std::vector<std::string> args = turbo::StrSplit(log_data, " ");
        int n;
        if (!turbo::SimpleAtoi(args[0], &n)) {
            return turbo::UnavailableError("");
        }
        type_ = static_cast<RedisType>(n);
