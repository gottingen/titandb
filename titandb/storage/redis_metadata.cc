...
// 52 bit for microseconds and 11 bit for counter
    ...
    uint64_t Metadata::generateVersion() {
        uint64_t timestamp = turbo::ToUnixMicros(turbo::Now());
    ...
    int64_t Metadata::TTL() const {
        ...
        auto now = turbo::ToUnixMillis(turbo::Now());
        if (expire < now) {
            return -2;
        }
    ...
    bool Metadata::Expired() const { return ExpireAt(turbo::ToUnixMillis(turbo::Now())); }
