...
namespace titandb {
    ...
    rocksdb::Status RedisStream::GetMetadata(const Slice &stream_name, StreamMetadata *metadata) {
        return RedisDB::GetMetadata(kRedisStream, stream_name, metadata);
    }

    rocksdb::Status RedisStream::GetLastGeneratedID(const Slice &stream_name, StreamEntryID *id) {
        std::string ns_key;
    ...
    StreamEntryID RedisStream::entryIDFromInternalKey(const rocksdb::Slice &key) const {
    ...
    std::string RedisStream::internalKeyFromEntryID(const std::string &ns_key, const StreamMetadata &metadata,
    ...
    rocksdb::Status RedisStream::Add(const Slice &stream_name, const StreamAddOptions &options,
                                const std::vector<std::string_view> &args, StreamEntryID *id) {
        ...
        if (!status.ok()) return rocksdb::Status::InvalidArgument(status.message());
    ...
    rocksdb::Status RedisStream::DeleteEntries(const rocksdb::Slice &stream_name, const std::vector<StreamEntryID> &ids,
                                          uint64_t *ret) {
        *ret = 0;
        ...
        auto iter = UniqueIterator(storage_, read_options, stream_cf_handle_);
...
// The entry with the ID `StreamLenOptions::entry_id` has not taken into account (it serves as exclusive boundary).
    rocksdb::Status RedisStream::Len(const rocksdb::Slice &stream_name, const StreamLenOptions &options, uint64_t *ret) {
        *ret = 0;
        std::string ns_key;
        AppendNamespacePrefix(stream_name, &ns_key);
    ...
    rocksdb::Status RedisStream::range(const std::string &ns_key, const StreamMetadata &metadata,
        ...
        if (start_key == end_key) {
            ...
            if (!rv.ok()) {
                return rocksdb::Status::InvalidArgument(rv.message());
            }
        ...
        for (; iter->Valid() &&
               (options.reverse ? iter->key().ToString() >= end_key : iter->key().ToString() <= end_key);
    ...
    rocksdb::Status RedisStream::getEntryRawValue(const std::string &ns_key, const StreamMetadata &metadata,
    ...
    rocksdb::Status
    RedisStream::GetStreamInfo(const rocksdb::Slice &stream_name, bool full, uint64_t count, StreamInfo *info) {
        std::string ns_key;
        AppendNamespacePrefix(stream_name, &ns_key);
    ...
    rocksdb::Status RedisStream::Range(const Slice &stream_name, const StreamRangeOptions &options,
    ...
    rocksdb::Status RedisStream::Trim(const rocksdb::Slice &stream_name, const StreamTrimOptions &options, uint64_t *ret) {
        *ret = 0;
    ...
    uint64_t RedisStream::trim(const std::string &ns_key, const StreamTrimOptions &options, StreamMetadata *metadata,
    ...
    rocksdb::Status RedisStream::SetId(const Slice &stream_name, const StreamEntryID &last_generated_id,
