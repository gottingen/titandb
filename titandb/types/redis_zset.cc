...
namespace titandb {
    ...
    rocksdb::Status RedisZSet::GetMetadata(const std::string_view &ns_key, ZSetMetadata *metadata) {
        return RedisDB::GetMetadata(kRedisZSet, ns_key, metadata);
    }

    rocksdb::Status RedisZSet::Add(const std::string_view &user_key, ZAddFlags flags, MemberScores *mscores, int *ret) {
        *ret = 0;
        ...
        for (int i = static_cast<int>(mscores->size() - 1); i >= 0; i--) {
            InternalKey(ns_key, (*mscores)[i].member, metadata.version, storage_->IsSlotIdEncoded()).Encode(
                    &member_key);
            ...
            if (metadata.size > 0) {
                ...
                if (s.ok()) {
                    ...
                    if (flags.HasIncr()) {
                        if ((flags.HasLT() && (*mscores)[i].score >= 0) ||
                            (flags.HasGT() && (*mscores)[i].score <= 0)) {
                            continue;
                        }
    ...
    rocksdb::Status RedisZSet::Card(const std::string_view &user_key, int *ret) {
        *ret = 0;
    ...
    rocksdb::Status RedisZSet::Count(const std::string_view &user_key, const RangeScoreSpec &spec, int *ret) {
        return RangeByScore(user_key, spec, nullptr, ret);
    }

    rocksdb::Status RedisZSet::IncrBy(const std::string_view &user_key, const std::string_view &member, double increment, double *score) {
        int ret = 0;
        std::vector<MemberScore> mscores;
        mscores.emplace_back(MemberScore{std::string(member), increment});
    ...
    rocksdb::Status RedisZSet::Pop(const std::string_view &user_key, int count, bool min, MemberScores *mscores) {
        mscores->clear();
        ...
        auto iter = UniqueIterator(storage_, read_options, score_cf_handle_);
        iter->Seek(start_key);
    ...
    rocksdb::Status
    RedisZSet::RangeByRank(const std::string_view &user_key, const RangeRankSpec &spec, MemberScores *mscores, int *ret) {
    ...
    rocksdb::Status
    RedisZSet::RangeByScore(const std::string_view &user_key, const RangeScoreSpec &spec, MemberScores *mscores, int *ret) {
        if (mscores) mscores->clear();
    ...
    rocksdb::Status RedisZSet::RangeByLex(const std::string_view &user_key, const RangeLexSpec &spec, Members *members, int *ret) {
    ...
    rocksdb::Status RedisZSet::Score(const std::string_view &user_key, const std::string_view &member, double *score) {
        std::string ns_key;
        AppendNamespacePrefix(user_key, &ns_key);
        ZSetMetadata metadata(false);
        rocksdb::Status s = GetMetadata(ns_key, &metadata);
    ...
    rocksdb::Status RedisZSet::Remove(const std::string_view &user_key, const std::vector<std::string_view> &members, int *ret) {
        *ret = 0;
        std::string ns_key;
        AppendNamespacePrefix(user_key, &ns_key);
    ...
    rocksdb::Status RedisZSet::Rank(const std::string_view &user_key, const std::string_view &member, bool reversed, int *ret) {
        *ret = -1;
    ...
    rocksdb::Status RedisZSet::Overwrite(const std::string_view &user_key, const MemberScores &mscores) {
        std::string ns_key;
        AppendNamespacePrefix(user_key, &ns_key);
    ...
    rocksdb::Status RedisZSet::InterStore(const std::string_view &dst, const std::vector<KeyWeight> &keys_weights,
    ...
    rocksdb::Status RedisZSet::UnionStore(const std::string_view &dst, const std::vector<KeyWeight> &keys_weights,
                                     AggregateMethod aggregate_method, int *size) {
        if (size) *size = 0;
    ...
    rocksdb::Status RedisZSet::Scan(const std::string_view &user_key, const std::string_view &cursor, uint64_t limit,
    ...
    rocksdb::Status RedisZSet::MGet(const std::string_view &user_key, const std::vector<std::string_view> &members,
        ...
        for (const auto &member: members) {
            ...
            double target_score = DecodeDouble(score_bytes.data());
            (*mscores)[std::string(member)] = target_score;
        }
        return rocksdb::Status::OK();
    }
