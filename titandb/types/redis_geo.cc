...
namespace titandb {
    ...
    rocksdb::Status RedisGeo::Add(const std::string_view &user_key, const std::vector<GeoPoint> &geo_points, int *ret) {
        ...
        return RedisZSet::Add(user_key, ZAddFlags::Default(), &member_scores, ret);
    }

    rocksdb::Status RedisGeo::Dist(const std::string_view &user_key, const std::string_view &member_1, const std::string_view &member_2, double *dist) {
        ...
        for (const auto &member: {member_1, member_2}) {
            auto iter = geo_points.find(member);
            if (iter == geo_points.end()) {
                return rocksdb::Status::NotFound();
            }
        }
        *dist =
                GeoHashHelper::GetDistance(geo_points[member_1].longitude,
                                           geo_points[member_1].latitude,
                                           geo_points[member_2].longitude,
                                           geo_points[member_2].latitude);
        return rocksdb::Status::OK();
    }

    rocksdb::Status RedisGeo::Hash(const std::string_view &user_key, const std::vector<std::string_view> &members,
        ...
        for (const auto &member: members) {
            auto iter = geo_points.find(std::string(member));
            if (iter == geo_points.end()) {
    ...
    rocksdb::Status RedisGeo::Pos(const std::string_view &user_key, const std::vector<std::string_view> &members,
                             turbo::flat_hash_map<std::string, GeoPoint> *geo_points) {
        return MGet(user_key, members, geo_points);
    }

    rocksdb::Status
    RedisGeo::Radius(const std::string_view &user_key, double longitude, double latitude, double radius_meters, int count,
        ...
        rocksdb::Status s = RedisZSet::GetMetadata(ns_key, &metadata);
        ...
        if (!store_key.empty()) {
            ...
            if (returned_items_count == 0) {
                ...
                int ret = 0;
                RedisZSet::Add(store_key, ZAddFlags::Default(), &member_scores, &ret);
            }
        }
    ...
    rocksdb::Status RedisGeo::RadiusByMember(const std::string_view &user_key, const std::string_view &member, double radius_meters, int count,
                                        DistanceSort sort, const std::string &store_key, bool store_distance,
                                        double unit_conversion, std::vector<GeoPoint> *geo_points) {
        GeoPoint geo_point;
    ...
    rocksdb::Status RedisGeo::Get(const std::string_view &user_key, const std::string_view &member, GeoPoint *geo_point) {
        turbo::flat_hash_map<std::string, GeoPoint> geo_points;
        auto s = MGet(user_key, {std::string(member)}, &geo_points);
        if (!s.ok()) return s;
    ...
    rocksdb::Status RedisGeo::MGet(const std::string_view &user_key, const std::vector<std::string_view> &members,
                                   turbo::flat_hash_map<std::string, GeoPoint> *geo_points) {
        turbo::flat_hash_map<std::string, double> member_scores;
        auto s = RedisZSet::MGet(user_key, members, &member_scores);
        if (!s.ok()) return s;

        for (const auto &member: members) {
            auto iter = member_scores.find(std::string(member));
            ...
            (*geo_points)[std::string(member)] = GeoPoint{xy[0], xy[1], std::string(member)};
        }
        return rocksdb::Status::OK();
    }

    std::string RedisGeo::EncodeGeoHash(double longitude, double latitude) {
    ...
    int RedisGeo::decodeGeoHash(double bits, double *xy) {
        GeoHashBits hash = {(uint64_t) bits, GEO_STEP_MAX};
        return GeohashDecodeToLongLatWGS84(hash, xy);
    }

/* Search all eight neighbors + self geohash box */
    int RedisGeo::membersOfAllNeighbors(const std::string_view &user_key, GeoHashRadius n, double lon, double lat, double radius,
 ...
 * Return the number of points added to the array. */
    int RedisGeo::membersOfGeoHashBox(const std::string_view &user_key, GeoHashBits hash, std::vector<GeoPoint> *geo_points, double lon,
                                 double lat, double radius) {
        GeoHashFix52Bits min = 0, max = 0;

        scoresOfGeoHashBox(hash, &min, &max);
        return getPointsInRange(user_key, static_cast<double>(min), static_cast<double>(max), lon, lat, radius,
                                geo_points);
    }
 ...
 * 'hash'. The two scores are returned by reference in *min and *max. */
    void RedisGeo::scoresOfGeoHashBox(GeoHashBits hash, GeoHashFix52Bits *min, GeoHashFix52Bits *max) {
    ...
    int RedisGeo::getPointsInRange(const std::string_view &user_key, double min, double max, double lon, double lat, double radius,
                              std::vector<GeoPoint> *geo_points) {
        ...
        int size = 0;
        std::vector<MemberScore> member_scores;
        rocksdb::Status s = RedisZSet::RangeByScore(user_key, spec, &member_scores, &size);
        if (!s.ok()) return 0;
 ...
 * returns true if the point is included, or false if it is outside. */
    bool
    RedisGeo::appendIfWithinRadius(std::vector<GeoPoint> *geo_points, double lon, double lat, double radius, double score,
    ...
    bool RedisGeo::sortGeoPointASC(const GeoPoint &gp1, const GeoPoint &gp2) { return gp1.dist < gp2.dist; }

    bool RedisGeo::sortGeoPointDESC(const GeoPoint &gp1, const GeoPoint &gp2) { return gp1.dist >= gp2.dist; }
