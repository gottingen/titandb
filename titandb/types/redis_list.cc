    ...
    rocksdb::Status RedisDB::Rem(const Slice &user_key, int count, const Slice &elem, int *ret) {
        *ret = 0;
        ...
        if (to_delete_indexes.size() == metadata.size) {
            ...
            uint64_t min_to_delete_index = !reversed ? to_delete_indexes[0] : to_delete_indexes[
                    to_delete_indexes.size() - 1];
            uint64_t max_to_delete_index = !reversed ? to_delete_indexes[to_delete_indexes.size() - 1]
                                                     : to_delete_indexes[0];
...
// Redis will treat it like the last element of the list.
    ...
    RedisDB::lmoveOnSingleList(const rocksdb::Slice &src, bool src_left, bool dst_left, std::string *elem) {
        ...
        InternalKey(ns_key, curr_index_buf, metadata.version).Encode(&curr_sub_key);
        ...
        InternalKey(ns_key, new_index_buf, metadata.version).Encode(&new_sub_key);
    ...
    rocksdb::Status RedisDB::lmoveOnTwoLists(const rocksdb::Slice &src, const rocksdb::Slice &dst, bool src_left,
        ...
        InternalKey(src_ns_key, src_buf, src_metadata.version).Encode(&src_sub_key);
        ...
        InternalKey(dst_ns_key, dst_buf, dst_metadata.version).Encode(&dst_sub_key);
