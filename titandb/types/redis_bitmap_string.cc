...
#include "titandb/common/type_util.h"

namespace titandb {

rocksdb::Status RedisBitmapString::GetBit(const std::string &raw_value, uint32_t offset, bool *bit) {
...
rocksdb::Status RedisBitmapString::SetBit(const Slice &ns_key, std::string *raw_value, uint32_t offset, bool new_bit,
                                     bool *old_bit) {
...
rocksdb::Status RedisBitmapString::BitCount(const std::string &raw_value, int64_t start, int64_t stop, uint32_t *cnt) {
  *cnt = 0;
  auto string_value = raw_value.substr(Metadata::GetOffsetAfterExpire(raw_value[0]));
...
rocksdb::Status RedisBitmapString::BitPos(const std::string &raw_value, bool bit, int64_t start, int64_t stop,
 ...
 * */
size_t RedisBitmapString::RawPopcount(const uint8_t *p, int64_t count) {
  size_t bits = 0;
 ...
 * */
int64_t RedisBitmapString::RawBitpos(const uint8_t *c, int64_t count, bool bit) {
  int64_t res = 0;
