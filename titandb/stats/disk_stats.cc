...
namespace titandb {
    ...
    rocksdb::Status Disk::GetHashSize(const Slice &ns_key, uint64_t *key_size) {
        ...
        rocksdb::Status s = RedisDB::GetMetadata(kRedisHash, ns_key, &metadata);
    ...
    rocksdb::Status Disk::GetSetSize(const Slice &ns_key, uint64_t *key_size) {
        ...
        rocksdb::Status s = RedisDB::GetMetadata(kRedisSet, ns_key, &metadata);
        if (!s.ok()) return s.IsNotFound() ? rocksdb::Status::OK() : s;
        return GetApproximateSizes(metadata, ns_key, storage_->GetCFHandle(kSubkeyColumnFamilyName), key_size);
    }
    ...
    rocksdb::Status Disk::GetListSize(const Slice &ns_key, uint64_t *key_size) {
        ...
        rocksdb::Status s = RedisDB::GetMetadata(kRedisList, ns_key, &metadata);
        if (!s.ok()) return s.IsNotFound() ? rocksdb::Status::OK() : s;
        std::string buf;
    ...
    rocksdb::Status Disk::GetSortedintSize(const Slice &ns_key, uint64_t *key_size) {
        ...
        rocksdb::Status s = RedisDB::GetMetadata(kRedisSortedint, ns_key, &metadata);
        if (!s.ok()) return s.IsNotFound() ? rocksdb::Status::OK() : s;
