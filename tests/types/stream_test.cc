...
TEST_F(RedisStreamTest, ParsingNextStreamEntryID) {
    CHECK_FALSE(ParseNextStreamEntryIDStrategy("XYX-ABC").ok());
    CHECK_FALSE(ParseNextStreamEntryIDStrategy("ABC-100").ok());
    CHECK_FALSE(ParseNextStreamEntryIDStrategy("100-ABC").ok());
    CHECK_FALSE(ParseNextStreamEntryIDStrategy("ABCDE").ok());
    CHECK_FALSE(ParseNextStreamEntryIDStrategy("-123456789").ok());
    CHECK_FALSE(ParseNextStreamEntryIDStrategy("123456789-").ok());
    CHECK_FALSE(ParseNextStreamEntryIDStrategy("*-XYZ").ok());
    CHECK_FALSE(ParseNextStreamEntryIDStrategy("ABC-*").ok());
    CHECK_FALSE(ParseNextStreamEntryIDStrategy("*-*").ok());
};
...
TEST_F(RedisStreamTest, NextIDSpecificTimestampAnySeqNumber) {
...

    {
        ...
        auto s = (*strategy)->GenerateID(last_id, &next_id);
        CHECK(s.ok());
        CHECK_EQ(next_id.ToString(), "100-101");
    }
...
TEST_F(RedisStreamTest, NextIDCurrentTimestampWithSpecificSeqNumber) {
...

    {
        ...
        CHECK(s.ok());
        EXPECT_GE(next_id.ms, ts);
        CHECK_EQ(next_id.seq, 123456789);
    }
...
TEST_F(RedisStreamTest, AddFirstEntryWithoutPredefinedID) {
    ...
    CHECK(s.ok());
    CHECK_EQ(id.seq, 0);
    CHECK(id.ms <= turbo::ToUnixMillis(turbo::Now()));
}

TEST_F(RedisStreamTest, AddEntryFirstEntryWithPredefinedID) {
    titandb::StreamEntryID expected_id{12345, 6789};
    titandb::StreamAddOptions options;
    options.next_id_strategy = *ParseNextStreamEntryIDStrategy(turbo::Format("{}-{}", expected_id.ms, expected_id.seq));
    std::vector<std::string_view> values = {"key1", "val1"};
    titandb::StreamEntryID id;
    auto s = stream_->Add(name_, options, values, &id);
    CHECK(s.ok());
    CHECK_EQ(id.ms, expected_id.ms);
    CHECK_EQ(id.seq, expected_id.seq);
}

TEST_F(RedisStreamTest, AddFirstEntryWithPredefinedNonZeroMsAndAnySeqNo) {
    uint64_t ms = turbo::ToUnixMillis(turbo::Now());
    titandb::StreamAddOptions options;
    options.next_id_strategy = *ParseNextStreamEntryIDStrategy(turbo::Format("{}-*", ms));
    std::vector<std::string_view> values = {"key1", "val1"};
    titandb::StreamEntryID id;
    auto s = stream_->Add(name_, options, values, &id);
    CHECK(s.ok());
...
TEST_F(RedisStreamTest, AddEntryToNonEmptyStreamWithPredefinedMsAndAnySeqNo) {
    titandb::StreamAddOptions options;
    options.next_id_strategy = *ParseNextStreamEntryIDStrategy(turbo::Format("{}-{}", 12345, 678));
...
TEST_F(RedisStreamTest, AddEntryWithPredefinedButExistingMsAndAnySeqNo) {
    ...
    uint64_t seq = 6789;
    titandb::StreamAddOptions options;
    options.next_id_strategy = *ParseNextStreamEntryIDStrategy(turbo::Format("{}-{}", ms, seq));
    std::vector<std::string_view> values = {"key1", "val1"};
    titandb::StreamEntryID id;
    auto s = stream_->Add(name_, options, values, &id);
    CHECK(s.ok());
...
TEST_F(RedisStreamTest, AddEntryWithTrimMaxLenStrategy) {
    ...
    CHECK_EQ(entries[0].key, id2.ToString());
    CheckStreamEntryValues(entries[0].values, values2);
    CHECK_EQ(entries[1].key, id3.ToString());
...
TEST_F(RedisStreamTest, AddEntryWithTrimMinIdStrategy) {
    titandb::StreamAddOptions add_options;
    add_options.next_id_strategy = *ParseNextStreamEntryIDStrategy(turbo::Format("{}-{}", 12345, 0));
    std::vector<std::string_view> values1 = {"key1", "val1"};
    titandb::StreamEntryID id1;
    auto s = stream_->Add(name_, add_options, values1, &id1);
    CHECK(s.ok());
    add_options.next_id_strategy = *ParseNextStreamEntryIDStrategy(turbo::Format("{}-{}", 12346, 0));
    std::vector<std::string_view> values2 = {"key2", "val2"};
    titandb::StreamEntryID id2;
    s = stream_->Add(name_, add_options, values2, &id2);
    CHECK(s.ok());
...
TEST_F(RedisStreamTest, AddEntryWithTrimMinIdStrategyThatDeletesAddedEntry) {
    ...
    add_options.trim_options = trim_options;
    add_options.next_id_strategy = *ParseNextStreamEntryIDStrategy(turbo::Format("{}-{}", 12347, 0));
    titandb::StreamEntryID id3;
    std::vector<std::string_view> values3 = {"key3", "val3"};
    s = stream_->Add(name_, add_options, values3, &id3);
    CHECK(s.ok());
...
TEST_F(RedisStreamTest, RangeWithStartAndEndSameMs) {
    titandb::StreamAddOptions add_options;
    add_options.next_id_strategy = *ParseNextStreamEntryIDStrategy(turbo::Format("{}-{}", 12345678, 0));
    std::vector<std::string_view> values1 = {"key1", "val1"};
    titandb::StreamEntryID id1;
    auto s = stream_->Add(name_, add_options, values1, &id1);
    CHECK(s.ok());
    add_options.next_id_strategy = *ParseNextStreamEntryIDStrategy(turbo::Format("{}-{}", 12345678, 1));
    std::vector<std::string_view> values2 = {"key2", "val2"};
    titandb::StreamEntryID id2;
    s = stream_->Add(name_, add_options, values2, &id2);
    CHECK(s.ok());
    add_options.next_id_strategy = *ParseNextStreamEntryIDStrategy(turbo::Format("{}-{}", 12345679, 0));
    std::vector<std::string_view> values3 = {"key3", "val3"};
    titandb::StreamEntryID id3;
    s = stream_->Add(name_, add_options, values3, &id3);
    CHECK(s.ok());
    ...
    CHECK(s.ok());
    CHECK_EQ(entries.size(), 2);
    CHECK_EQ(entries[0].key, id1.ToString());
    CheckStreamEntryValues(entries[0].values, values1);
    CHECK_EQ(entries[1].key, id2.ToString());
    CheckStreamEntryValues(entries[1].values, values2);
}

TEST_F(RedisStreamTest, RangeInterval) {
    titandb::StreamAddOptions add_options;
    add_options.next_id_strategy = *ParseNextStreamEntryIDStrategy(turbo::Format("{}-{}", 123456, 1));
    std::vector<std::string_view> values1 = {"key1", "val1"};
    titandb::StreamEntryID id1;
    auto s = stream_->Add(name_, add_options, values1, &id1);
    CHECK(s.ok());
    add_options.next_id_strategy = *ParseNextStreamEntryIDStrategy(turbo::Format("{}-{}", 123457, 2));
    std::vector<std::string_view> values2 = {"key2", "val2"};
    titandb::StreamEntryID id2;
    s = stream_->Add(name_, add_options, values2, &id2);
    CHECK(s.ok());
    add_options.next_id_strategy = *ParseNextStreamEntryIDStrategy(turbo::Format("{}-{}", 123458, 3));
    std::vector<std::string_view> values3 = {"key3", "val3"};
    titandb::StreamEntryID id3;
    s = stream_->Add(name_, add_options, values3, &id3);
    CHECK(s.ok());
    add_options.next_id_strategy = *ParseNextStreamEntryIDStrategy(turbo::Format("{}-{}", 123459, 4));
    ...
    CHECK_EQ(entries[1].key, id2.ToString());
    CheckStreamEntryValues(entries[1].values, values2);
    CHECK_EQ(entries[2].key, id3.ToString());
...
TEST_F(RedisStreamTest, RangeFromMinimumToMaximum) {
    ...
    CHECK_EQ(entries[2].key, id3.ToString());
    CheckStreamEntryValues(entries[2].values, values3);
    CHECK_EQ(entries[3].key, id4.ToString());
...
TEST_F(RedisStreamTest, RevRangeWithStartAndEndSameMs) {
    ...
    CHECK_EQ(entries[0].key, id2.ToString());
    CheckStreamEntryValues(entries[0].values, values2);
    CHECK_EQ(entries[1].key, id1.ToString());
...
TEST_F(RedisStreamTest, RevRangeInterval) {
    ...
    CHECK(s.ok());
    CHECK_EQ(entries.size(), 3);
    CHECK_EQ(entries[0].key, id3.ToString());
    CheckStreamEntryValues(entries[0].values, values3);
    CHECK_EQ(entries[1].key, id2.ToString());
    CheckStreamEntryValues(entries[1].values, values2);
    CHECK_EQ(entries[2].key, id1.ToString());
...
TEST_F(RedisStreamTest, RevRangeFromMaximumToMinimum) {
    ...
    CHECK(s.ok());
    CHECK_EQ(entries.size(), 4);
    CHECK_EQ(entries[0].key, id4.ToString());
    CheckStreamEntryValues(entries[0].values, values4);
    CHECK_EQ(entries[1].key, id3.ToString());
    CheckStreamEntryValues(entries[1].values, values3);
    CHECK_EQ(entries[2].key, id2.ToString());
    CheckStreamEntryValues(entries[2].values, values2);
    CHECK_EQ(entries[3].key, id1.ToString());
...
TEST_F(RedisStreamTest, DeleteMultipleEntries) {
    ...
    CHECK_EQ(entries[0].key, id2.ToString());
    CheckStreamEntryValues(entries[0].values, values2);
    CHECK_EQ(entries[1].key, id4.ToString());
...
TEST_F(RedisStreamTest, StreamInfoOnEmptyStream) {
    ...
    CHECK_EQ(info.last_generated_id.ToString(), id.ToString());
    CHECK_EQ(info.max_deleted_entry_id.ToString(), id.ToString());
    CHECK_EQ(info.entries_added, 1);
    CHECK_EQ(info.recorded_first_entry_id.ToString(), "0-0");
    CHECK_FALSE(info.first_entry);
    CHECK_FALSE(info.last_entry);
}
...
TEST_F(RedisStreamTest, StreamInfoOneEntry) {
    ...
    CHECK_EQ(info.last_generated_id.ToString(), id.ToString());
    CHECK_EQ(info.max_deleted_entry_id.ToString(), "0-0");
    CHECK_EQ(info.entries_added, 1);
    CHECK_EQ(info.recorded_first_entry_id.ToString(), id.ToString());
    CHECK(info.first_entry);
    CHECK_EQ(info.first_entry->key, id.ToString());
    CheckStreamEntryValues(info.first_entry->values, values);
    CHECK(info.last_entry);
    CHECK_EQ(info.last_entry->key, id.ToString());
...
TEST_F(RedisStreamTest, StreamInfoOnStreamWithElements) {
    ...
    CHECK_EQ(info.last_generated_id.ToString(), id3.ToString());
    CHECK_EQ(info.max_deleted_entry_id.ToString(), "0-0");
    CHECK_EQ(info.entries_added, 3);
    CHECK_EQ(info.recorded_first_entry_id.ToString(), id1.ToString());
    CHECK(info.first_entry);
    CHECK_EQ(info.first_entry->key, id1.ToString());
    CheckStreamEntryValues(info.first_entry->values, values1);
    CHECK(info.last_entry);
    CHECK_EQ(info.last_entry->key, id3.ToString());
    CheckStreamEntryValues(info.last_entry->values, values3);
    CHECK_EQ(info.entries.size(), 0);
}
...
TEST_F(RedisStreamTest, StreamInfoOnStreamWithElementsFullOption) {
    ...
    CHECK_FALSE(info.first_entry);
    CHECK_FALSE(info.last_entry);
    CHECK_EQ(info.entries.size(), 3);
    CHECK_EQ(info.entries[0].key, id1.ToString());
    CheckStreamEntryValues(info.entries[0].values, values1);
    CHECK_EQ(info.entries[1].key, id2.ToString());
    CheckStreamEntryValues(info.entries[1].values, values2);
    CHECK_EQ(info.entries[2].key, id3.ToString());
...
TEST_F(RedisStreamTest, StreamInfoCheckAfterLastEntryDeletion) {
    ...
    CHECK_EQ(info.last_generated_id.ToString(), id3.ToString());
    CHECK_EQ(info.max_deleted_entry_id.ToString(), id3.ToString());
    CHECK_EQ(info.entries_added, 3);
    CHECK_EQ(info.recorded_first_entry_id.ToString(), id1.ToString());
    CHECK(info.first_entry);
    CHECK_EQ(info.first_entry->key, id1.ToString());
    CheckStreamEntryValues(info.first_entry->values, values1);
    CHECK(info.last_entry);
    CHECK_EQ(info.last_entry->key, id2.ToString());
...
TEST_F(RedisStreamTest, StreamInfoCheckAfterFirstEntryDeletion) {
    ...
    CHECK_EQ(info.size, 2);
    CHECK_EQ(info.last_generated_id.ToString(), id3.ToString());
    CHECK_EQ(info.max_deleted_entry_id.ToString(), id1.ToString());
    CHECK_EQ(info.entries_added, 3);
    CHECK_EQ(info.recorded_first_entry_id.ToString(), id2.ToString());
    CHECK(info.first_entry);
    CHECK_EQ(info.first_entry->key, id2.ToString());
...
TEST_F(RedisStreamTest, StreamInfoCheckAfterTrimMinId) {
    ...
    CHECK(s.ok());
    CHECK_EQ(info.size, 2);
    CHECK_EQ(info.last_generated_id.ToString(), id4.ToString());
    CHECK_EQ(info.max_deleted_entry_id.ToString(), id2.ToString());
    CHECK_EQ(info.entries_added, 4);
    CHECK_EQ(info.recorded_first_entry_id.ToString(), id3.ToString());
    CHECK(info.first_entry);
    CHECK_EQ(info.first_entry->key, id3.ToString());
    CheckStreamEntryValues(info.first_entry->values, values3);
    CHECK(info.last_entry);
    CHECK_EQ(info.last_entry->key, id4.ToString());
...
TEST_F(RedisStreamTest, StreamInfoCheckAfterTrimAllEntries) {
    ...
    CHECK_EQ(info.size, 0);
    CHECK_EQ(info.last_generated_id.ToString(), id4.ToString());
    CHECK_EQ(info.max_deleted_entry_id.ToString(), id4.ToString());
    CHECK_EQ(info.entries_added, 4);
    CHECK_EQ(info.recorded_first_entry_id.ToString(), "0-0");
    CHECK_FALSE(info.first_entry);
    CHECK_FALSE(info.last_entry);
...
TEST_F(RedisStreamTest, StreamSetIdNonExistingStreamCreatesEmptyStream) {
    ...
    CHECK(s.ok());
    CHECK_EQ(info.last_generated_id.ToString(), last_id.ToString());
    CHECK_EQ(info.entries_added, entries_added);
    CHECK_EQ(info.max_deleted_entry_id.ToString(), max_del_id->ToString());
...
TEST_F(RedisStreamTest, StreamSetIdLastIdGreaterThanExisting) {
    ...
    s = stream_->GetStreamInfo(name_, false, 0, &info);
    CHECK(s.ok());
    CHECK_EQ(info.entries_added, added);
