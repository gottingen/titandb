...
protected:
    explicit RedisListTest() { list_ = std::make_unique<titandb::RedisList>(storage_, "list_ns"); }
...
TEST_F(RedisListLMoveTest, LMoveSrcAndDstAreTheSameManyElemsRightLeft) {
    ...
    EXPECT_TRUE(s.ok());
    EXPECT_EQ(elem, fields_[fields_.size() - 1]);
    listElementsAreEqualTo(key_, 0, static_cast<int>(fields_.size() + 1),
...
TEST_F(RedisListTest, RPopOneElement) {
    ...
    for (size_t i = 0; i < fields_.size(); i++) {
        std::string elem;
        list_->Pop(key_, false, &elem);
        EXPECT_EQ(elem, fields_[fields_.size() - i - 1]);
    }
    std::string elem;
...
TEST_F(RedisListTest, RPopMulti) {
    ...
    EXPECT_EQ(elems.size(), requested_size);
    for (size_t i = 0; i < elems.size(); ++i) {
        EXPECT_EQ(elems[i], fields_[fields_.size() - i - 1]);
    }
    list_->Del(key_);
}
